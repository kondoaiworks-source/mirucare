...
6. ルールブック閲覧で国・県または自治体を選び、ルール一覧とカバー率が出ること。根拠情報は別ページで国／県／市の読むPDFとリンク集を分け、本文が無いPDFはリンクを直せること。未登録の根拠カテゴリが推奨として出ること
...
4. 下書き画面にカバー率・参照資料数（国・県／市区町村）・ルール構成・「保存」待ちが出ること。ルールの修正・追加・下書きから外す・確定する／全て破棄するができること。公式資料から出したルールは「公式資料」と出ること
5. 確定するまで施設の書類チェックに新しいルールが載らないこと。国・県の確定後は作成へ戻り、市の確定後は閲覧へ進むこと
6. 確定後、ルールブック閲覧の「国・県」に共通ルールだけ、「横浜市」に横浜市のルールだけが出ること。領域フィルタが無いこと
7. ルールブック閲覧でルールの保存・追加・停止・削除ができること。根拠情報ページで国／県／市の読むPDFと参考リンクが置け、本文なしのPDFは「リンクを直す」できること。各資料に「修正する」「ルールブックへ追加」があること
...
11. ルールブック閲覧 → 横浜市でカバー率と「監査観点の確認状況」が出ること。了承済み／根拠あり／了承待ち／未作成の件数と、観点ごとの不足理由・次にやることが確認できること
