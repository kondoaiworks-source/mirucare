...
7. 定期実行（本番）: Vercel Cron `0 15 * * *`（UTC＝毎日0:00 JST頃）→ `/api/cron/knowledge-sync`
   ...
   - ローカル確認例:
     ...
     ```
8. 設定画面の運営カードから「ルール設定」へ遷移できること（行政ルール台帳・マニュアル変更の承認はルール設定内）
...
4. 左メニューが次の構成であること
   ...
   - 自治体管理（自治体マスタ）
   - 監視トラブル（同期結果をこの画面で表示。行政ルール台帳へのリンクあり）
...
13. 更新アラートまたは行政ルール台帳の資料から「判定ルール案を生成する」→ `/admin/rules/pending` に案＋根拠が載り、了承するまでチェックに使われないこと
14. 「行政ルール台帳を開く」で `/admin/rules/documents?city=...` が開き、監視状況（OK／NG／差分あり）と登録済み台帳があること
15. [http://localhost:3000/admin/rules/more](http://localhost:3000/admin/rules/more) が「監視トラブル」で、未解消アラートと最終同期一覧を直接表示すること
16. 監視トラブル内の「行政ルール台帳」で `/admin/rules/documents?register=1` が開き、登録フォームが出ること
...
20. 市ルールブックのチェックルールに「手入力で判定ルールを1件追加する」があり、登録後は新ルール判定通知に載ること
...
5. 監視トラブルの「行政ルール台帳」で `/admin/rules/documents?register=1` が開き、登録フォームが出ること
6. 行政ルール台帳の「監視状況」に OK／NG／差分ありが出ること（最大5件＋全部見る）
...
3. 設定（運営アカウント）の運営向けに「レビューコンソールを開く」「ルール設定」のみあること（行政ルール台帳・マニュアル変更の承認の直リンクが無いこと）
...
| `/admin/rules/more` | 監視トラブル（同期結果を直接表示・行政ルール台帳リンク） |
| `/admin/rules/jobs` | 監視トラブルへリダイレクト |
| `/admin/rules/documents` | 行政ルール台帳（監視状況・台帳・例外登録）。旧 `/admin/documents` はリダイレクト |
