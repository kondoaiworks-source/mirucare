...
7. 市ルールブックは「チェックルール → 新ルール判定 → 自治体ルール設定」の順で、件数カードや参照URL／資料の二重一覧がないこと
8. 自治体ルール設定で国／県／市が折りたたみ表示され、参照URL・行政資料から判定ルール案を生成できること
9. 市ルールブックで新ルール判定（更新アラート）を「台帳へ反映する／判定ルール案を生成する／差し戻す／解消する」できること
