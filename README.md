...
5. 訪問介護 → ルールブックを作る を開く
   ...
   - 停止中の領域が選択肢に出ないこと
6. 下書き画面で案内文の修正・追加・下書きから外す・確定ができること。件数は「国・県／市固有」と出ること。市のPDFから出したルールは「市資料」と出ること
7. 確定するまで施設の書類チェックに新しいルールが載らないこと
8. 確定後、ルールブックを見る（横浜市）で国・県の共通ルール＋横浜市のルールが出ること
9. ルールブックを見るで案内文の保存・追加・停止・削除と、資料先（国／県／市）が開けること
10. SQL Editor で `supabase/migrations/20260814120000_compose_origin_city_pdf.sql` を適用する。「下書きを作る」で国・県の標準観点と市固有が一緒に出ること。市のPDFが未同期でも標準の観点だけで下書きが作れること。市対応前の下書きが残っているときは破棄して作り直すこと
