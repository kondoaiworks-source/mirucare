...
6. ルールブック閲覧で国・県または自治体を選び、ルール一覧が出ること。根拠情報に根拠カテゴリとカテゴリカバー率が出ないこと。国・県・市の読むPDFとリンク集だけであること
...
10. 旧URL `/admin/rules/pending`・`/admin/rules/manual`・`/admin/rules/history` が利用設定へリダイレクトすること。`/admin/rules/domains` も利用設定へリダイレクトすること。`/admin/rules/source-urls` が根拠情報へリダイレクトすること
...
7. ルールブック閲覧で登録済みルールの保存ボタンが無いこと。停止・削除と、画面下の「ルールを追加する」ができること。一番下の「訪問介護に戻る」でハブへ戻れること。根拠情報で国／県／市に資料を追加するとき、根拠カテゴリを選ぶ欄が無いこと
...
10. 書類チェックを実行し、適用ルール版に基本突合以外の承認済み頻出観点ルール（例：契約・同意、感染症BCP、事故報告など）が、書類種別を問わず含まれること
11. ルールブック閲覧 → 横浜市で国・県・市の読むPDF有無（カバー率）と確定済みルール一覧が出ること。初回登録・監査観点の確認状況・頻出観点の基本確認・次にやることは出ないこと
...
> 2026-08 変更: 「何をチェックするか」の種類選択を廃止し、**まとめてアップロードして開始**する導線に変更しました。チェック時は了承済みルールブック全体を参照し、書類種別では絞りません。
...
3. 一緒に見る書類をまとめてドロップ／選択、またはスマホで撮影する
   - 種類選択カードが出ないこと
   - 進捗バーが表示され、失敗時に「再試行」ができること
   - `doc_type` はファイル名からの推定（内部用）になること
4. 原本の取り扱いに同意し、「同意して運用AI監査を開始する」を押す
5. 結果画面（または `/documents` 一覧）へ遷移し、状態が「チェック中」になること
6. 一覧で「今日の分／過去の分」と空状態CTAを確認する
7. **チェック未開始の取り消し**: アップだけ済ませて離脱すると一覧に「チェック未開始」が残る。カードの「このアップロードを取り消す」で一覧・ナビバッジから消えること（完了ボタンでは消えない）
   - 先に SQL で `20260714020000_cancel_uploaded_document_rpc.sql` を実行する（未実行だと RPC 無しで service role にフォールバック）
   - スマホでも取消ボタンが押せること（カード全体のリンクではなく独立ボタン）。「続けてチェックする」から同じ書類のアップ画面へ再開できること
   - ウィザードで一覧の×を押したあと再アップしても、旧の「チェック未開始」が増えないこと（ローカル削除時にDBも取消）
8. 受け入れ条件の目安
   - 375px幅で「アップ→開始」がスムーズに進むこと
   - 大きめPDFでも進捗が見えること（画面遷移しても UploadProvider により継続）
   - Storage バケットは private。閲覧は署名付きURL（10分）のみ

## 動作確認手順（チェック時のルールブック全体参照）

1. ルールブック「見る」で了承済み（確定）が数十件ある自治体（例：横浜市）の事業所でログインする
2. `/check/upload` で種類を選ばず書類をアップし、チェックを開始する
3. 結果画面の「このチェックで使った基準」が、標準観点＋了承済みルールで **約48件前後（上限60＋標準1）** になること（標準1件だけにならないこと）
4. 指摘が「記録の整合性チェック」「ルール適合チェック」に分かれること（分類未設定ばかりにならないこと）
...
4. `/check/upload` からまとめてアップロードしてチェックする（先頭の結果画面だけがセットを実行する）
5. 結果に「訪問介護計画の日付がケアプラン更新に追いついていない可能性があります」が **書類同士のご確認** に出ること（「不適合」とは出ない）
6. 日付が片方しか無い、または計画の日付が同じ／新しい場合は、この指摘が出ないこと
7. 「このチェックで使った基準」に「計画の更新日の確認」など標準観点が出ること（ルールブック未確定でも載る）
8. 複数ファイルのときは「今回一緒に入れた N件」と出ること。Vercel / 開発ログに `[check] set_alignment_catalog` と `setSize` / `catalogCount` が出ること（日付・氏名は出さない）

## 動作確認手順（セット内の整合性カタログ拡張）

同じチェックセットの本文から、日付以外の機械突合も行います（断定せず「可能性」として提示）。

1. `npx vitest run src/lib/check/alignment-catalog.test.ts src/lib/check/service-time-overlap.test.ts src/lib/check/record-before-plan.test.ts src/lib/check/consent-date-alignment.test.ts` が PASS すること
2. 次の文言が読めるPDF／テキストを、まとめて `/check/upload` からチェックする
   - **提供時間の重複**: 同じ日に `13:00～14:00` と `13:30～14:30` など重なる時間帯
   - **計画前の提供日**: 訪問介護計画の作成日より前の「サービス提供日／実施日」
   - **同意日と開始日**: 「同意日」が「サービス開始日／利用開始日」より後
3. 結果の「記録の整合性チェック」に、該当する指摘が出ること（比較欄に見比べた書類名と日付・時間が付くこと）
4. 「このチェックで使った基準」に次の標準観点が載ること（了承済みルールとは別）
   - 計画の更新日の確認
   - 提供時間帯の重複の確認
   - 計画前の提供日の確認
   - 同意日と開始日の確認
5. 片方しか日付が無い・重ならない場合は、その観点の指摘が出ないこと
...
3. `/check/upload` でこのPDFをアップロードしてチェックする
...
11. 適用ルール版パネルに、対象自治体の了承済み頻出観点ルールが出ること（書類種別では絞らない。従来の基本突合だけに絞る検証は `CHECK_RULES_SCOPE=phase1`）
12. 運営で頻出観点ルールを投入する:
...
2. 根拠カテゴリとカテゴリカバー率が出ないこと。国／県／市のいずれかで「資料を追加する」→ 「読むPDF」または「参考リンク（HTML）」を選んで資料名とURLを登録する（カテゴリ選択は無い）
...
10. 根拠情報に自動監視の注意文が出ること（カテゴリカバー率は出ない）
...
3. 「書類をまとめてチェックする」画面が直接開き、種類選択カードが出ないこと
4. チェック完了後の結果画面（`/check/[id]`）では **監査結果** が選択中になること
