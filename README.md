...
2. 運営アカウントで `/admin/rules/services/homecare/municipalities` を開く（旧 `/admin/rules/municipalities` からもリダイレクト）
...
2. `/admin/rules/services`（介護サービス選定）へリダイレクトすること
3. 左メニューが「介護サービス選定／ルール管理／公開情報台帳管理／監視トラブル」であること
4. 訪問介護 → 国・県ルール設定で国・県の公開情報を追加できること
5. 訪問介護 → 市区町村ルール設定で横浜・川崎・藤沢・鎌倉・茅ヶ崎が一覧され、運用／停止できること
6. 市の設定 → 監査カテゴリ設定で4チェック（ケアプラン⇔計画書／計画⇔実績／シフト⇔提供記録／国保連請求⇔実績）があること
7. 各カテゴリ詳細に「関連PDFの確認」「【更新情報管理】」「ルール設定」があり、生成・了承は「ルール管理」へ誘導すること
8. `/admin/rules/pending`（ルール管理）上部で台帳資料から「判定ルール案を生成する」ができ、生成後に了承待ちへ載ること（URL登録だけでは載らない）
   - 「本文あり」バッジの資料から生成できること
   - 「再同期で補完します」の資料は、生成時に PDF を再取得してスナップショットを補完すること（Storage `knowledge-snapshots` が無いと失敗メッセージになる）
   - 同期が 304 / ハッシュ一致でもスナップショット欠落時は本体を取り直し、保存失敗時は `last_sync_status=failed` になること
   - 「手入力で判定ルールを1件追加する（API不要）」から1件載せ、了承待ちに出ること
9. `20260731060000_rule_category_pdf_candidates.sql` を適用したうえで、カテゴリ詳細の「関連PDFを検索する」が動くこと
10. 候補のリンクで内容を確認し、「採用」で台帳監視が始まり、「不採用」で一覧から外れること
11. URLを手で追加した候補も同様に採用／不採用できること
12. 旧URL `/admin/rules/regulatory`・`/admin/rules/regulatory/[slug]`・`/admin/rules/municipalities` が新階層へリダイレクトすること
13. 市・国県画面には生成ボタンが無く、ルール管理への導線があること
...
## 動作確認手順（ルール設定：ルールブック中心）

> 2026-07-31：入口は **介護サービス選定**（`/admin/rules/services`）に変更。詳細は上記「サービス起点 IA」と [docs/ルールブック構想.md](docs/ルールブック構想.md)。
...
4. 左メニューが次の構成であること
   - 介護サービス選定／ルール管理／公開情報台帳管理
   - 監視トラブル（同期結果をこの画面で表示。公開情報監視へのリンクあり）
   - 「更新履歴」はナビに単独項目がなく、ルール管理ページ内にあること
5. `/admin/rules` が介護サービス選定（`/admin/rules/services`）へリダイレクトすること
6. 訪問介護 → 市区町村から各市を開けること
7. 監査カテゴリ（4チェック）と横断ルール管理が使えること
8. 更新アラートまたは公開情報監視の資料から「判定ルール案を生成する」→ `/admin/rules/pending` に案＋根拠が載り、了承するまでチェックに使われないこと
9. [http://localhost:3000/admin/rules/more](http://localhost:3000/admin/rules/more) が「監視トラブル」であること
10. `/admin/rules/jobs` が監視トラブルへリダイレクトすること
11. 判定ルール一覧（`/admin/rules/ai-rules`）に新規登録フォームが無いこと
12. findings のレビューコンソール（`/admin`）や Stripe には影響しないこと
13. 判定ルール案の生成には `GEMINI_API_KEY` が必要（未設定時はエラーメッセージが出ること）
14. 施設の自治体が Phase1市のとき、他市専用の了承ルールはチェックに載らず、国・県・自市・共通は載ること
...
5. 監視トラブルの「公開情報監視」で `/admin/rules/documents?register=1` が開き、登録フォームが出ること
...
13. `/admin/rules/jobs` が監視トラブルへリダイレクトすること
...
| `/admin/rules/regulatory` | ルールブック管理ハブ |
| `/admin/rules/regulatory/[slug]` | Phase1市ルールブック（ルールブック管理導線） |
| `/admin/rules/pending` | ルール管理（了承待ち＋更新履歴） |
| `/admin/rules/history` | ルール管理の更新履歴へリダイレクト |
| `/admin/rules/notifications` | 公開情報台帳管理 |
| `/admin/rules/more` | 監視トラブル（同期結果を直接表示・公開情報監視リンク） |
| `/admin/rules/jobs` | 監視トラブルへリダイレクト |
| `/admin/rules/documents` | 公開情報監視（監視状況・台帳・例外登録）。旧 `/admin/documents` はリダイレクト |
