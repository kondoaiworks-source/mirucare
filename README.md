...
8. `20260731060000_rule_category_pdf_candidates.sql` を適用したうえで、カテゴリ詳細の「関連PDFを検索する」が動くこと
9. 候補のリンクで内容を確認し、「採用」で台帳監視が始まり、「不採用」で一覧から外れること
10. URLを手で追加した候補も同様に採用／不採用できること
11. 旧URL `/admin/rules/regulatory`・`/admin/rules/regulatory/[slug]`・`/admin/rules/municipalities` が新階層へリダイレクトすること
12. `/admin/rules/pending` の横断了承キューが従来どおり使えること
