...
7. ホーム冒頭に「このホームで何を準備するのか」が表示されること
   - 「何を見るか」「どう見ればよいか」「何を根拠にするか」の3つが、介護以外の例も含めて説明されること
   - 「目的から選ぶ」の各カードに「登録するデータ」「何に使うか」が表示されること
8. ホーム最上部に「準備の進み具合」があること
   - 必須 / 任意のチェックリストと「次にやること」が表示されること
   - 状態ラベルが「未設定」「準備中」「利用可能」「要対応あり」のいずれかであること
   - 必須完了後に「利用可能」になり、任意未完了でも次アクションが任意へ進むこと
9. 監査対策で項目を1件登録 → AI判定ルールを「承認待ち」で登録 → 承認待ちで理由を書いて承認できること
   - ホームの必須進捗と目的カードの「設定あり／未完了」が更新されること
10. findings のレビューコンソール（`/admin`）や Stripe には影響しないこと
11. 監査対策で訪問介護テンプレートを一括登録できること
