...
6. 「ルールブック設定」に Phase1市の案内と参照URL／行政資料／更新アラートへのリンクがあること
7. [http://localhost:3000/admin/rules/more](http://localhost:3000/admin/rules/more) が「詳細設定」であること
8. ホーム最上部に「準備の進み具合」があること
   - 必須 / 任意のチェックリストと「次にやること」が表示されること
   - 状態ラベルが「未設定」「準備中」「利用可能」のいずれかであること
9. 詳細設定から監査項目を1件登録 → 判定ルールを「承認待ち」で登録 → 承認待ちで理由を書いて承認できること
10. findings のレビューコンソール（`/admin`）や Stripe には影響しないこと
11. 監査項目で訪問介護テンプレートを一括登録できること（詳細設定内）
