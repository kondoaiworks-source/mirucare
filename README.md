...
7. 市ルールブック先頭付近に「チェック用の中身（了承済み判定ルール）」があり、了承前の案も併記されること
8. 市ルールブック先頭に「確定版の目次」（国→県→市）があり、参照URL・行政資料が層別に並ぶこと
9. 市ルールブックで更新アラートを「台帳へ反映する／判定ルール案を生成する／差し戻す／解消する」できること
10. 市ルールブックで「参照URLを追加する」「修正する」「確認済みにする」ができること
11. 更新アラートまたは行政資料から「判定ルール案を生成する」→ `/admin/rules/pending` に案＋根拠が載り、了承するまでチェックに使われないこと
12. 「この市の行政資料を編集する」で `/admin/rules/documents?city=...` が開き、一覧が市に絞られること
