...
4. 国／県／市の折りたたみで参照URLの追加・修正・削除・確認済み化ができること
...
9. 「文言を修正する」から新ルール判定通知へ修正案を載せられ、了承前はチェックに使われないこと
...
13. 更新アラートまたは行政資料から「判定ルール案を生成する」→ `/admin/rules/pending` に案＋根拠が載り、了承するまでチェックに使われないこと
14. 「行政資料台帳を開く」で `/admin/rules/documents?city=...` が開き、一覧が市に絞られること
15. [http://localhost:3000/admin/rules/more](http://localhost:3000/admin/rules/more) が「監視トラブル」で、行政資料台帳／同期の結果のみであること
16. ルールブック設定に「初回セットアップ」があり、監査項目＋Phase1判定ルールを登録できること
17. 市ルールブックのチェックルールに「手入力で判定ルールを1件追加する」があり、登録後は新ルール判定通知に載ること
18. 判定ルール一覧（`/admin/rules/ai-rules`）に新規登録フォームが無いこと
19. findings のレビューコンソール（`/admin`）や Stripe には影響しないこと
20. 判定ルール案の生成には `GEMINI_API_KEY` が必要（未設定時はエラーメッセージが出ること）
21. 施設の自治体が Phase1市のとき、他市専用の了承ルールはチェックに載らず、国・県・自市・共通は載ること
...
8. 結果画面に匿名化の注意文があること
9. 設定 →「事業所の設定を変更する」→ `/setup` で Phase1 対象市を選べること
...
4. 監視トラブル → 行政資料に、同名／同URLの台帳が自動で増えていること
5. `rule_sources.knowledge_document_id` が埋まっていること（SQLまたは編集画面）
6. HTML／一覧ページのみ（PDF直リンクなし）の場合は台帳には載るが、画面上の注意どおり「以降の変更を自動監視」までは行かないことがあること
7. 市ルールブックの自治体ルール設定に「自動監視について」の注意文が出ること
8. 「削除する」で一覧から外れ（`status=archived`）、再表示されないこと
9. ヘッダー右上に「ログイン中の事業所」と、設定した表示名が出ること
...
5. 主要画面（運用AI監査・あとで確認・監査結果・設定・お知らせ・使い方・事業所の設定）の表題サイズが揃っていること（h1: 2xl / md:3xl）
...
| `/admin/rules/more` | 監視トラブル（行政資料台帳・同期の結果） |
| `/admin/rules/jobs` | 同期の結果（監視トラブル内） |
