...
2. 運営アカウントで `/admin/rules/municipalities` を開く
3. 「公開設定（サービス × 自治体）」で訪問介護タブを開き、Phase1市が公開中であること
4. 通所介護タブでは非公開であること（施設の登録画面に通所介護が出ないこと）
5. 国・県・市のいずれかに公開情報PDFが無い市は「公開する」が押せないこと（共通層前提）
6. 公開中の市を「非公開にする」→ 既存施設の設定画面ではその市が「現在の設定・公開終了済み」として残ること
7. 新規オンボーディングでは非公開の市が選べないこと
...
4. 左メニューが次の構成であること
   - ルールブック管理／ルール管理／公開情報台帳管理
   - 自治体管理（自治体マスタ）
   - 監視トラブル（同期結果をこの画面で表示。公開情報監視へのリンクあり）
   - 「更新履歴」はナビに単独項目がなく、ルール管理ページ内にあること
   - 詳細設定（監査項目・判定ルール）は主導線に出ないこと（市ルールブック／初回セットアップへ）
5. `/admin/rules` がルールブック管理（`/admin/rules/regulatory`）へリダイレクトすること
6. 「ルールブック管理」に Phase1市カードがあり、各市ルールブックが開けること（重複ガイド・サービス選択UIは出ないこと）
7. 市ルールブックは「チェックルール → 新ルール判定 → 自治体ルール設定」がそれぞれ枠付きのひとまとまりで、見出しの文字サイズ・レイアウトが揃っていること
8. チェックルールはジャンル（契約・計画・記録など）ごとに折りたたみ表示され、「原本の根拠を開く」「文言を修正する」があること
9. 「文言を修正する」からルール管理へ修正案を載せられ、了承前はチェックに使われないこと
10. 自治体ルール設定で国／県／市が折りたたみ表示され、各層で公開情報の追加・修正・削除ができること（「更新アラート管理」に更新アラートの確認だけがあり、公開情報登録・法令・根拠が無いこと）
11. 市ルールブックで新ルール判定を「台帳へ反映する／判定ルール案を生成する／差し戻す／解消する」できること
12. 空の新ルール判定は「現在判定案件は0件です。」と出ること
13. 更新アラートまたは公開情報監視の資料から「判定ルール案を生成する」→ `/admin/rules/pending` に案＋根拠が載り、了承するまでチェックに使われないこと
14. 「公開情報監視を開く」で `/admin/rules/documents?city=...` が開き、監視状況（OK／NG／差分あり）と登録済み台帳があること
15. [http://localhost:3000/admin/rules/more](http://localhost:3000/admin/rules/more) が「監視トラブル」で、未解消アラートと最終同期一覧を直接表示すること
16. 監視トラブル内の「公開情報監視」で `/admin/rules/documents?register=1` が開き、登録フォームが出ること
17. `/admin/rules/jobs` が監視トラブルへリダイレクトすること
18. ルールブック管理に「初回登録の手順」があり、7ステップの進捗・国県市の公開情報表・Phase1突合（項目1・3・7・8）の網羅表で抜け漏れを目視できること
19. 各市ルールブック（`/admin/rules/regulatory/[slug]`）上部に「○○市の初回登録」があり、市ごとの手順・Phase1網羅表・該当セクションへのジャンプリンクがあること
20. 市ルールブックのチェックルールに「手入力で判定ルールを1件追加する」があり、登録後はルール管理に載ること
21. 判定ルール一覧（`/admin/rules/ai-rules`）に新規登録フォームが無いこと
22. findings のレビューコンソール（`/admin`）や Stripe には影響しないこと
23. 判定ルール案の生成には `GEMINI_API_KEY` が必要（未設定時はエラーメッセージが出ること）
24. 施設の自治体が Phase1市のとき、他市専用の了承ルールはチェックに載らず、国・県・自市・共通は載ること
