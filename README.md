...
5. 訪問介護ハブが「ルールブックを作る／ルールブックを見る／資料先」の3枠であること
6. ルールブックを見るで国・県または自治体と領域を選び、ルール一覧が出ること。資料先は別ページで国／県／市の公式URLを開き、本文が無いものはリンクを直せること
...
9. `/admin/rules/monitoring`（監視状況）がサイドナビのもう1本であること。差分ありの詳細から「確認してルールブックを作り直す」、エラーから「資料先でリンクを直す」へ進めること
...
6. 下書き画面で案内文の修正・追加・下書きから外す・確定ができること。件数は「国・県／市固有」と出ること。公式資料から出したルールは「公式資料」と出ること。案内文が「どの書類の何を見比べるか」になっていること（「観点です」だけのメモになっていないこと）
7. 確定するまで施設の書類チェックに新しいルールが載らないこと。国・県の確定後は作るへ戻り、市の確定後は見るへ進むこと
8. 確定後、ルールブックを見るの「国・県」に共通ルールだけ、「横浜市」に横浜市のルールだけが出ること
9. ルールブックを見るで案内文の保存・追加・停止・削除ができること。資料先ページで国／県／市の公式URLが開け、本文なしは「リンクを直す」できること
10. SQL Editor で `supabase/migrations/20260814120000_compose_origin_city_pdf.sql` と `20260814140000_compose_origin_official.sql` と `20260815040000_compose_extraction_notes.sql` を適用する。「下書きを作る」で国・県・市の公式資料から観点が出ること。本文がある資料は1件ずつ読むこと。一部の資料で抽出に失敗しても、出せた観点は了承画面に残ること。了承画面に層ごとの抽出結果（資料件数／本文件数／出せた件数または出せなかった理由）が出ること。本文0件の層は「資料先でリンクを確認する」へ進めること。資料が無い層だけ標準の見比べ観点で穴埋めされること。古い「観点です」下書きが残っているときは破棄して作り直すこと。ボタンは「資料を読んでいます…」になり、数分かかることがあること
...
8. 監視状況でエラー印 → 詳細、資料先でリンクを直す／差分ありならルールブックを作り直すへ進めること
...
1. 運営アカウントで訪問介護の資料先（`/admin/rules/services/homecare/sources`）を開く
2. 国／県／市のいずれかで「公開情報を追加する」→ 資料名・PDF直リンクを登録する
3. 成功メッセージに「監視を開始」系の文言が出ること
4. 監視状況（`/admin/rules/monitoring`）にエラー／差分／正常のサマリとリストが出ること
5. 差分ありの詳細から「原文を開く」「確認してルールブックを作り直す」へ進めること。作り直し先は訪問介護の「ルールブックを作る」（市が付くこと）。エラーの詳細から「資料先でリンクを直す」へ進めること
