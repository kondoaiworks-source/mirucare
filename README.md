...
3. 読むPDFの成功メッセージに監視開始の案内が出ること。HTMLは「リンク集に載せました」となり、抽出・本文なしに入らないこと。大きい公式PDFは追加・今すぐ同期に数分かかることがあること（最大約5分）
4. 監視状況（`/admin/rules/monitoring`）にエラー／差分／正常のサマリとリストが出ること（読むPDFのみ）。PDFは取れたが本文0件のときは正常にせずエラーになり、再同期で本文を取り直すこと
