...
7. 各カテゴリ詳細に「関連PDFの確認」「【更新情報管理】」「ルール設定」があり、横断の「ルール管理」へリンクがあること
8. 市の設定または国・県ルール設定の「判定ルール案を生成する」で案が `/admin/rules/pending` に載ること（URL登録だけでは載らない）
...
13. `/admin/rules/pending` の横断了承キューが従来どおり使えること
