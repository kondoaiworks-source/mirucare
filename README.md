...
16. ルールブック設定に「初回登録の手順」があり、7ステップの進捗・国県市の参照URL表・Phase1突合（項目1・3・7・8）の網羅表で抜け漏れを目視できること
17. 各市ルールブック（`/admin/rules/regulatory/[slug]`）上部に「○○市の初回登録」があり、市ごとの手順・Phase1網羅表・該当セクションへのジャンプリンクがあること
18. 市ルールブックのチェックルールに「手入力で判定ルールを1件追加する」があり、登録後は新ルール判定通知に載ること
19. 判定ルール一覧（`/admin/rules/ai-rules`）に新規登録フォームが無いこと
20. findings のレビューコンソール（`/admin`）や Stripe には影響しないこと
21. 判定ルール案の生成には `GEMINI_API_KEY` が必要（未設定時はエラーメッセージが出ること）
22. 施設の自治体が Phase1市のとき、他市専用の了承ルールはチェックに載らず、国・県・自市・共通は載ること
