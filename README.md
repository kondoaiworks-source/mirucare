...
5. 訪問介護 → ルールブックを作る を開く
   ...
   - 停止中の領域が選択肢に出ないこと
6. 下書き画面で案内文の修正・追加・下書きから外す・確定ができること。件数は「国・県／市固有」と出ること。市のPDFから出したルールは「市資料」と出ること。市固有が0件なら「市の資料からルールを出す」で再取得できること
7. 確定するまで施設の書類チェックに新しいルールが載らないこと
8. 確定後、ルールブックを見る（横浜市）で国・県の共通ルール＋横浜市のルールが出ること
9. ルールブックを見るで案内文の保存・追加・停止・削除と、資料先（国／県／市）が開けること
10. SQL Editor で `supabase/migrations/20260814120000_compose_origin_city_pdf.sql` を適用する。市のPDFが未同期でも標準の観点だけで下書きが作れること。市固有が国・県しか出ないときは、下書きを破棄せず「市の資料からルールを出す」を押すか、同じ3つで下書きを作り直すこと（市PDF対応前の下書きは国・県だけ残っている）
