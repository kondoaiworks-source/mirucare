...
7. 市ルールブックは「チェック用の中身 → 更新アラート → 根拠の目次」の順で、件数カードや参照URL／資料の二重一覧がないこと
8. 根拠の目次（国→県→市）に参照URL・行政資料が並び、行政資料から判定ルール案を生成できること
9. 市ルールブックで更新アラートを「台帳へ反映する／判定ルール案を生成する／差し戻す／解消する」できること
