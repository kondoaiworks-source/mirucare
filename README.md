...
15. [http://localhost:3000/admin/rules/more](http://localhost:3000/admin/rules/more) が「監視トラブル」で、連携監視のみであること（同期の結果は連携監視へリダイレクト）
16. 連携監視で「手動登録」を押すと登録フォームが開き、登録済み台帳が一覧できること
17. ルールブック設定に「初回登録の手順」があり、7ステップの進捗・国県市の参照URL表・Phase1突合（項目1・3・7・8）の網羅表で抜け漏れを目視できること
18. 各市ルールブック（`/admin/rules/regulatory/[slug]`）上部に「○○市の初回登録」があり、市ごとの手順・Phase1網羅表・該当セクションへのジャンプリンクがあること
19. 市ルールブックのチェックルールに「手入力で判定ルールを1件追加する」があり、登録後は新ルール判定通知に載ること
20. 判定ルール一覧（`/admin/rules/ai-rules`）に新規登録フォームが無いこと
21. findings のレビューコンソール（`/admin`）や Stripe には影響しないこと
22. 判定ルール案の生成には `GEMINI_API_KEY` が必要（未設定時はエラーメッセージが出ること）
23. 施設の自治体が Phase1市のとき、他市専用の了承ルールはチェックに載らず、国・県・自市・共通は載ること
...
4. 監視トラブル → 連携監視に、同名／同URLの台帳が自動で増えていること
5. 連携監視の「監視状況」に OK／NG／差分ありが出ること（最大5件＋全部見る）
6. NG・差分ありをタップすると詳細が開き、再同期や差分確認へ進めること
7. 「手動登録」で登録フォームが開くこと
...
13. `/admin/rules/jobs` が `/admin/rules/documents?view=all` へリダイレクトすること
...
| `/admin/rules/more` | 監視トラブル（連携監視） |
| `/admin/rules/jobs` | 連携監視へリダイレクト（`?view=all`） |
