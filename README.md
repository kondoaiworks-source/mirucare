...
4. 左メニューが次の構成であること
   - ホーム
   - 設定する：監査項目 / AI判定ルール / 行政情報
   - 確認する：承認待ち / 更新履歴
   - その他の設定（加算・自治体・ジョブなどはここへ）
5. ホームに「よく使う設定」3カードと「その他の設定」への案内があること
6. [http://localhost:3000/admin/rules/more](http://localhost:3000/admin/rules/more) から加算・ジョブ等へ辿れること
7. ホーム最上部に「準備の進み具合」があること
   - 必須 / 任意のチェックリストと「次にやること」が表示されること
   - 状態ラベルが「未設定」「準備中」「利用可能」のいずれかであること
   - 必須完了後に「利用可能」になり、任意未完了でも次アクションが任意へ進むこと
8. 監査項目で項目を1件登録 → AI判定ルールを「承認待ち」で登録 → 承認待ちで理由を書いて承認できること
   - ホームの必須進捗と目的カードの「設定あり／未完了」が更新されること
9. findings のレビューコンソール（`/admin`）や Stripe には影響しないこと
10. 監査項目で訪問介護テンプレートを一括登録できること
   - 先に「ルールセットを選ぶ」で訪問介護のセットを選択する
   - 「訪問介護テンプレートを登録する」で最大公約数の86項目がまとめて登録されること
   - 同じボタンを再度押しても、登録済み項目はスキップされ重複エラーにならないこと
   - 個別登録ではコードを空欄のまま登録でき、自動採番されたコードが一覧に表示されること
...
| `/admin/rules/more` | その他の設定（加算・自治体・ジョブなど） |
