...
7. 市ルールブックで更新アラートを「台帳へ反映する／差し戻す／解消する」できること
8. 「この市の参照URLを編集する」で `/admin/rules/source-urls?city=...` が開き、自治体フィルタが掛かること
9. 「この市の行政資料を編集する」で `/admin/rules/documents?city=...` が開き、一覧が市に絞られること
10. [http://localhost:3000/admin/rules/more](http://localhost:3000/admin/rules/more) が「詳細設定」であること
11. ホーム最上部に「準備の進み具合」があること
12. 詳細設定から監査項目を1件登録 → 判定ルールを「承認待ち」で登録 → 承認待ちで理由を書いて承認できること
13. findings のレビューコンソール（`/admin`）や Stripe には影響しないこと
14. 監査項目で訪問介護テンプレートを一括登録できること（詳細設定内）
