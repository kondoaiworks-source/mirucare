...
3. **ステップ1（何をチェックしますか？）**: 目的カードを1つ選ぶ
   - 「日報・提供記録をチェック／ケアプランをチェック／勤務表をチェック／請求データをチェック／その他の書類をチェック」から選択
   - 選ぶと自動でステップ2へ進むこと
...
7. 定期実行（本番）: Vercel Cron `0 15 * * *`（UTC＝毎日0:00 JST頃）→ `/api/cron/knowledge-sync`
   ...
   - ローカル確認例:
     ...
     ```
8. 設定画面の運営カードから「行政資料を開く」または「ルール設定を開く」へ遷移できること
...
2. 運営アカウントでログインする
3. 設定 → 「ルール設定を開く」または [http://localhost:3000/admin/rules](http://localhost:3000/admin/rules)
4. 左メニューが次の構成であること
   - ホーム
   - ルールブック設定
   - 承認待ち / 更新履歴
   - 自治体マスタ / 通知一覧 / 運用監視
   - 詳細設定（監査項目・判定ルール・加算など）
5. ホームに「ルールブックから始める」とメニューカードがあること
...
8. 自治体ルール設定で国／県／市が折りたたみ表示され、各行の「URLを修正する／資料を修正する」および見出しの「参照URLを修正する」から編集画面へ進めること
9. 市ルールブックで新ルール判定を「台帳へ反映する／判定ルール案を生成する／差し戻す／解消する」できること
10. チェックルールは了承済み件数付きで折りたたみ、空の新ルール判定は「現在判定案件は0件です。」と出ること
11. `/admin/rules/source-urls?city=kawasaki&edit=（参照URLのID）` で当該行の編集フォームが開くこと
10. 「この市の参照URLを追加・修正する」を開くとインライン追加・編集ができること
11. 更新アラートまたは行政資料から「判定ルール案を生成する」→ `/admin/rules/pending` に案＋根拠が載り、了承するまでチェックに使われないこと
12. 「行政資料を編集する」で `/admin/rules/documents?city=...` が開き、一覧が市に絞られること
13. [http://localhost:3000/admin/rules/more](http://localhost:3000/admin/rules/more) が「詳細設定」であること
14. ホーム最上部に「準備の進み具合」があること
15. 詳細設定から監査項目を1件登録 → 判定ルールを「承認待ち」で登録 → 承認待ちで理由を書いて承認できること
16. findings のレビューコンソール（`/admin`）や Stripe には影響しないこと
17. 監査項目で訪問介護テンプレートを一括登録できること（詳細設定内）
18. 判定ルール案の生成には `GEMINI_API_KEY` が必要（未設定時はエラーメッセージが出ること）
19. 施設の自治体が Phase1市のとき、他市専用の了承ルールはチェックに載らず、国・県・自市・共通は載ること
