...
6. ルールブックを見るで自治体・領域を選び、ルール一覧が出ること。資料先は別ページで国／県／市の公式URLが開けること
...
9. ルールブックを見るで案内文の保存・追加・停止・削除ができること。資料先ページで国／県／市の公式URLが開けること
10. SQL Editor で `supabase/migrations/20260814120000_compose_origin_city_pdf.sql` と `20260814140000_compose_origin_official.sql` と `20260815040000_compose_extraction_notes.sql` を適用する。「下書きを作る」で国・県・市の公式資料から観点が出ること。了承画面に層ごとの抽出結果（資料件数／本文件数／出せた件数または出せなかった理由）が出ること。資料が無い層だけ標準の見比べ観点で穴埋めされること。古い「観点です」下書きが残っているときは破棄して作り直すこと
