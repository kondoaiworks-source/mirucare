...
- [ ] サイドバーに「運用AI監査」「設定」があり、「法令／運営AI」が準備中であること（人手確認）
...
- [ ] ハンバーガーに「使い方」（初期設定・設定・アップロードは無し）
- [ ] サイドバーに「初期設定」が無いこと
- [ ] `/guide` に使い方要約と注意事項があること

### 3-B. 事業所の設定

- [ ] 設定 → 事業所情報 →「事業所の設定を変更する」で `/setup` が開く
- [ ] `/setup` で横浜・川崎・藤沢・鎌倉・茅ヶ崎を選べる
- [ ] 管理者で自治体を保存できる
...
| 川崎が事業所設定に出ない／ルール管轄がおかしい | 1-B 未実行、または rule_engine 未適用 |
