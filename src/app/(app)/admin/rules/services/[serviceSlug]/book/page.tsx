...
import { ViewRulebookAdmin } from "@/components/features/admin/rules/view-rulebook-admin"
...
export default async function ViewRulebookPage({
  params,
  searchParams,
}: PageProps) {
  const { serviceSlug } = await Promise.resolve(params)
  ...
  return (
    <Suspense fallback={<p className="text-base text-muted-foreground">読み込み中です。</p>}>
      <ViewRulebookAdmin
        service={service}
