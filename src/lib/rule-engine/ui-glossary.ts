 ...
 *
 * 階層の正:
 * 利用設定 → サービス設定／領域マスタ／自治体マスタ
 * サービス → ルールブックを作る／ルールブックを見る／資料先
 * 本線: 資料先にPDF直URLを置く → 監視 → 作る → 人が直して確定 → 見る
 * 根拠: 資料先（国／県／市の公式PDF。本文が無いときは人が直す）
 * チェック: 国・県の共通ルール ＋ その市のルール
 *
 * 動線ルール:
 * - 1画面1作業。横飛びリンクは置かない
 * - 例外: 了承画面で本文0件→資料先。監視で差分あり→作る。監視でエラー→資料先
